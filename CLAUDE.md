...
| `<Plug>(HexPairModified)` reached `README.md` and `hexpair.vimrc` and never the help - no tag, and no line in the help's own copy of the mapping set. Within one cycle, as was `g:hexpair_scan_block` missing from `hexpair.vimrc`, and the `README.md` of the new minimal package linking to the three files that package omits. All three are the same failure: what is added LAST is added to whichever files the author had open | "every option the plugin reads is listed in all four places", "every <Plug> target and highlight group is listed everywhere", "a shipped document links only to what ships beside it" |
