...
| **Four search checks compared their neighbour's answer, on any machine that has hexpair INSTALLED.** The `tbfind.vim` block was the one generated script calling the autoload functions by name, and the one without a hermetic `runtimepath`. `-u NONE` does not stop an autoload lookup: Vim searches the package start directories under `'packpath'` as well as `'runtimepath'`, so the first bare `hexpair#FindForward()` reached `~/.vim/pack/*/start/hexpair/autoload/hexpair.vim` and sourced a SECOND copy of the Vim9 script `HexPairPagedBlobFindSupported()` had already sourced by path — **E1073: Name already defined** on every export. The call itself still returned the right answer, which is why nothing looked broken; the error aborted the rest of the enclosing `:if`, one `call add()` never ran, and every later line of `tbfind.out` moved up one, so four `sed -n Np` checks read the line below theirs. The maintainer's own machine has the plugin installed, so this failed there and passed in CI | `let &runtimepath = '$ROOT,' . $VIMRUNTIME` in that block, like the four `filetype plugin on` blocks already had it: pointed at the repo, the lookup finds the file the plugin already sourced and does nothing |
